...
contract StakingPool is Context, Ownable, ReentrancyGuard {
  ...
  constructor(address _token, uint256[] _lockupPeriod, uint256[] _percentage) {
    token = _token;
    for (uint8 _i; _i < _lockupPeriod.length; _i++) {
        pools[_i].lockupPeriod = _lockupPeriod[_i];
    }
    pools[0].percentage = (FACTOR * 1.5) / 100;
    pools[1].percentage = (FACTOR * 12.5) / 100;
  ...
  function _addShares(uint256 pid, address wallet, uint256 amount) private {
    ...
    rewards[wallet][pid].excluded = _cumulativeRewards(pid, shares[wallet][pid].amount);
  }

  function _removeShares(uint256 pid, address wallet, uint256 amount) private {
    require(
      shares[wallet][pid].amount > 0 && amount <= shares[wallet][pid].amount,
      'REM: amount'
    );
    require(
      block.timestamp > shares[wallet][pid].stakedTime + lockupPeriod[pid],
      'REM: timelock'
    );
  ...
  function getUnpaid(uint256 pid, address wallet) public view returns (uint256) {
    ...
    uint256 earnedRewards = _cumulativeRewards(pid, shares[wallet][pid].amount);
